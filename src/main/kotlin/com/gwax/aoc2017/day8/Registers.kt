...
enum class RegisterOperation(val repr: String) {
    DEC("dec") { override fun perform(r: Int, v: Int) = r - v },
    INC("inc") { override fun perform(r: Int, v: Int) = r + v };

    abstract fun perform(r: Int, v: Int): Int

    companion object Factory {
        fun fromRepr(repr: String) = values().find { it.repr == repr } ?: throw IllegalArgumentException(repr)
    }
}

enum class RegisterComparison(val repr: String) {
    EQ("==") { override fun perform(r: Int, v: Int) = r == v },
    NE("!=") { override fun perform(r: Int, v: Int) = r != v },
    GT(">") { override fun perform(r: Int, v: Int) = r > v },
    GE(">=") { override fun perform(r: Int, v: Int) = r >= v },
    LT("<") { override fun perform(r: Int, v: Int) = r < v },
    LE("<=") { override fun perform(r: Int, v: Int) = r <= v };

    abstract fun perform(r: Int, v: Int): Boolean
    companion object Factory {
        fun fromRepr(repr: String) = values().find { it.repr == repr } ?: throw IllegalArgumentException(repr)
    }
}

val instReprRegex = "([a-z]+) (inc|dec) (-?\\d+) if ([a-z]+) ([=!><]=|[><]) (-?\\d+)".toRegex()

data class Instruction(
        val targetRegister: String,
        val operation: RegisterOperation,
        val operand: Int,
        val evalRegister: String,
        val comparator: RegisterComparison,
        val comparand: Int) {

    companion object Factory {
        fun fromRepr(repr: String): Instruction {
            val groups = instReprRegex.find(repr)?.groupValues ?: throw IllegalArgumentException(repr)
            return Instruction(
                    groups[1],
                    RegisterOperation.fromRepr(groups[2]),
                    groups[3].toInt(10),
                    groups[4],
                    RegisterComparison.fromRepr(groups[5]),
                    groups[6].toInt(10))
        }
    }
}
...
fun processInstructions(instructions: List<Instruction>): Pair<Map<String, Int>, Int> {
    ...
    instructions.forEach {
        ...
        val comparison = it.comparator.perform(eval, it.comparand)
        if (comparison)
            registerBank[it.targetRegister] = it.operation.perform(
                    registerBank.getOrDefault(it.targetRegister, 0),
                    it.operand)
        maxMax = max(maxMax, registerBank[it.targetRegister] ?: 0)
    }
    return Pair(registerBank.toMap(), maxMax)
}

fun main(args: Array<String>) {
    val instructions = input.lines().map { Instruction.fromRepr(it) }
    val (registerBank, maxSeen) = processInstructions(instructions)
