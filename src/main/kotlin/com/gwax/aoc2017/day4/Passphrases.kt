...
fun validPassphrase(phrase: String, sorted: Boolean): Boolean {
    var words = phrase.split("\\s+".toRegex())
    if (sorted) words = words.map {
        it.toList().sorted().joinToString(separator = "")
    }
    return words.size == words.distinct().size
}

fun countValidPassphrases(input: String, sorted: Boolean): Int =
        input.lines()
                .map { validPassphrase(it, sorted) }
                .count { it }

fun main(args: Array<String>) {
    println(countValidPassphrases(input, false))
    println(countValidPassphrases(input, true))
}
