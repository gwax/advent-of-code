...
fun TowerProgram.mergeProgream(other: TowerProgram) {
    ...
    this.parent = this.parent ?: other.parent
    this.children.addAll(other.children)
}

val nodeRegex = "^([a-z]+) \\((\\d+)\\)(?: -> (.+))?$".toRegex(RegexOption.MULTILINE)

fun parseNodes(input: String): List<Node> {
    return nodeRegex
            .findAll(input)
            .map { it.destructured }
            .map { (name, weight, children) ->
                Node(
                        ...
                        TowerProgram(
                                ...
                                when (children) {
                                    "" -> mutableListOf()
                                    else -> children.split(",").map { it.trim() }.toMutableList()
                                }))
            }.toList()
}
...
fun towerWeight(root: ProgramName, nodeMap: Map<ProgramName, TowerProgram>): Int {
    val rootNode = nodeMap[root] ?: return 0
...
fun fixWeight(root: ProgramName, nodeMap: Map<ProgramName, TowerProgram>, target: Int): Int {
    ...
    if (childToWeight.values.distinct().size == 1)
        return target - childToWeight.values.sum()

    val differentChild = childToWeight
            .filterValues { !childToWeight.values.minus(it).contains(it) }
            .keys
            .first()
    val newTarget = childToWeight
            .filterValues { childToWeight.values.minus(it).contains(it) }
            .values
            .first()
    return fixWeight(differentChild, nodeMap, newTarget)
}

fun main(args: Array<String>) {
    val nodeMap = buildNodeMap(input)
    val rootName = nodeMap.filter { (_, v) -> v.parent == null }.keys.first()
    println(rootName)
    print(fixWeight(rootName, nodeMap, 0))
}
